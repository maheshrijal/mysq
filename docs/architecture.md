...
The `Context` interface includes the versioned JSON shape, collection timestamp and interval, explicit probe capabilities, server identity, raw counters, normalized statements, evidence tables, deterministic findings, and health score. Renderers do not recompute facts. Exports do not re-query MySQL. History does not retain connection configuration.
...
An inspection uses one connection and samples `SHOW GLOBAL STATUS` twice around the configured interval. All other work happens before that rate window and is bounded catalog or Performance Schema access with fixed limits: 20 statement digests, 100 tables, 100 processes, and no application-row queries. `MAX_EXECUTION_TIME` is 10 seconds and the session is pinned read-only. Lock data is explicitly point-in-time and can change while the report is being consumed.
