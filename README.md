...
- Ghostty is the primary terminal target. Body text and backgrounds use terminal defaults; accents use the terminal's ANSI palette, so changing Ghostty's theme also updates already-rendered colors. There is no cached light/dark RGB palette to become stale. SQL keywords, identifiers, numeric evidence, warnings, and destructive actions have distinct roles; essential labels retain the terminal's foreground contrast.
- Main Connections, Queries, Engine, and Tables grids use cyan identities, magenta metrics, and blue SQL keywords. Amber highlights lock waits, pending metadata locks, missing primary keys, and hidden-index flags. Color supplements the displayed values; selected rows use terminal-native reverse video.
...
- Overview adds live graphs for queries/sec, running threads, row-lock waits/sec, and physical InnoDB read/write bytes/sec. A separate one-query sampler runs every two seconds while the TUI is open, retaining up to five minutes in memory. Large terminals show a 2×2 grid; smaller layouts use sparklines below the summary (`G` scrolls to them). `p` pauses/resumes sampling and freezes the timeline.
- Graphs share a time axis, with zero-based scales, current values, and window peaks. Startup collects history; failed samples, counter resets, and pauses leave gaps. The sampler excludes its own query and running thread; rates otherwise describe server-wide activity. These samples do not update diagnostic findings, enter history/exports, or change the `--interval` diagnostic sampling window. Use `r` for a new diagnostic snapshot.
...
- Cancellation stays bound to the control connection that observed the execution. A lost connection or server restart invalidates the selection, even if execution IDs are reused; refresh and select again. The TUI retains at most one control connection after opening live query details and closes it on exit.
